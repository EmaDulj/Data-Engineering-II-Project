...
def train_model(config):
    ...
    model = CatBoostRegressor(
        ...
        objective=config["objective"], #'RMSE' 
        #objective = config["objective"],
        use_best_model=True)
    
    model.fit(X_train_scaled, y_train, eval_set=(X_test_scaled,y_test),plot=False)
...
config = {
    "iterations": tune.choice([50, 40, 60]),
    "depth": tune.choice([13, 14, 15, 16]),
    "learning_rate": tune.choice([0.1, 0.05, 0.3, 0.2]),
    "objective": tune.choice(["RMSE", "MAE", "Quantile", ])
